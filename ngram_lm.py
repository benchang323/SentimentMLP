...
def plot_next_word_prob(word_scores, word_candidates, context, top=10, save_path=None):
    ...
    top_candidates = list(word_candidates[context])
    top_scores = list(word_scores[context])

    indices = np.argsort(top_scores)[-top:][::-1]
    top_candidates = [top_candidates[i] for i in indices]
    top_scores = [top_scores[i] for i in indices]
    
    plt.bar(top_candidates, top_scores)
    plt.xlabel('Next Word')
    plt.ylabel('Probability')
    
    plt.title(f'Top-10 Most Probable Next Tokens')
    plt.tight_layout()
