...
class SentimentClassifier(nn.Module):
    ...
    def softmax(logits):
        ...
        e_logits = torch.exp(logits - torch.max(logits, dim=1, keepdim=True)[0])
        sum_logits = torch.sum(e_logits, dim=1, keepdim=True)
        return e_logits / sum_logits
    ...
    def gradient_loss(self, inp, logits, labels):
        ...
        # Gradient of loss -> logits
        grad_logits = (softmax_logits - torch.eye(self.num_classes)[labels]) / bsz
        
        # Gradient of loss -> weights
        grads_weights = torch.matmul(grad_logits.t(), inp)
        
        # Gradient of loss -> bias
        grads_bias = torch.sum(grad_logits, dim=0)
